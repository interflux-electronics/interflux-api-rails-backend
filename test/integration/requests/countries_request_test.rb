...
class CountriesRequestTest < ActionDispatch::IntegrationTest
  ...
  test 'Public users can fetch all countries' do
    get '/countries', headers: public_header
  ...
  test 'Public users can fetch a single country by ID' do
    get "/countries/#{@belgium.id}", headers: public_header
    data = JSON.parse(@response.body)['data']
    assert_response 200
  ...
  test 'Public users cannot create countries' do
    post '/countries', headers: public_header
    assert_response 403
  end

  test 'Public users cannot update countries' do
    put "/countries/#{@belgium.id}", headers: public_header
    assert_response 403
  end

  test 'Public users cannot delete countries' do
    delete "/countries/#{@belgium.id}", headers: public_header
    assert_response 403
  end
end

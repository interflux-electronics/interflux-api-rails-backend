...
module V1
  module Admin
    class UserIntegrationTest < ApplicationController
      # Public users should be able to fetch all users
      test '1' do
        assert_can_fetch_all true, 5, 6
      end

      # Public users should be able to fetch one user by ID
      test '2' do
        assert_can_fetch_one_by_id true
      end

      # Public users should be able to fetch one user by slug
      test '3' do
        assert_can_fetch_one_by_slug true
      end

      # Public users should NOT be able to create users
      test '4' do
        assert_can_create false
      end

      # Public users should NOT be able to update users
      test '5' do
        assert_can_update false
      end

      # Public users should NOT be able to delete users
      test '6' do

...
class AdminProductSubGroupsIntegrationTest < ActionDispatch::IntegrationTest
  def setup
  ...
  test 'Authorized admin users can fetch all product groups' do
    get '/admin/product-sub-groups', headers: @authorized_header
    data = JSON.parse(@response.body)['data']
    assert_response 200
    assert_equal data.length, 16, 'Should return 16 product groups'
  end
